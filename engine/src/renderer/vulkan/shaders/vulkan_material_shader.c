...
b8 vulkan_material_shader_create(vulkan_context* context, vulkan_material_shader* out_shader)
{
    ...
    VkShaderStageFlagBits stage_type[MATERIAL_SHADER_STAGE_COUNT] = { VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_FRAGMENT_BIT };

    for(u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i)
    {
        if(!create_shader_module(context, BUILTIN_SHADER_NAME_MATERIAL, stage_type_strs[i], stage_type[i], i, out_shader->stages))
        {
    ...
    object_pool_size[0].descriptorCount = VULKAN_MAX_MATERIAL_COUNT;
    // The second section will be used for image samplers
    object_pool_size[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    object_pool_size[1].descriptorCount = VULKAN_MATERIAL_SHADER_SAMPLER_COUNT * VULKAN_MAX_MATERIAL_COUNT;
    ...
    object_pool_info.pPoolSizes = object_pool_size;
    ...
    VK_CHECK(vkCreateDescriptorPool(context->device.logical_device, &object_pool_info, context->allocator, &out_shader->object_desdcriptor_pool));
...
#define ATTRIBUTE_COUNT 2
    ...
    if(!vulkan_buffer_create(
        ...
        VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, //  VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
        true,
...
void vulkan_material_shader_destroy(vulkan_context* context, struct vulkan_material_shader* shader)
{
    ...
    vkDestroyDescriptorPool(logical_device, shader->object_desdcriptor_pool, context->allocator);
...
void vulkan_material_shader_apply_material(vulkan_context* context, struct vulkan_material_shader* shader, material* material)
{
    if (context && shader) {
        ...
        u64 offset = sizeof(vulkan_material_shader_instance_ubo) * 4 * material->internal_id; // also the index into the array
...
b8 vulkan_material_shader_acquire_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material)
{
    ...
    VkDescriptorSetAllocateInfo alloc_info = { VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO };
    alloc_info.descriptorPool = shader->object_desdcriptor_pool;
...
void vulkan_material_shader_release_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material)
{
    ...
    VkResult result = vkFreeDescriptorSets(context->device.logical_device, shader->object_desdcriptor_pool, descriptor_set_count, instance_state->descriptor_sets);
    if(result != VK_SUCCESS)
    {
