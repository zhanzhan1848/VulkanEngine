...
b8 vulkan_material_shader_create(vulkan_context* context, vulkan_material_shader* out_shader)
{
    ...
    VkDescriptorType descriptor_types[VULKAN_OBJECT_MAX_OBJECT_COUNT] = {
    ...
    object_pool_size[0].descriptorCount = VULKAN_OBJECT_MAX_OBJECT_COUNT;
    // The second section will be used for image samplers
    object_pool_size[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    object_pool_size[1].descriptorCount = local_sampler_count * VULKAN_OBJECT_MAX_OBJECT_COUNT;
...
void vulkan_material_shader_update_object(vulkan_context* context, struct vulkan_material_shader* shader, geometry_render_data data)
{
    ...
    {
        texture* t = data.texture[sampler_index];
...
void vulkan_material_shader_release_resources(vulkan_context* context, struct vulkan_material_shader* shader, u32 object_id)
{
    ...
    VkResult result = vkFreeDescriptorSets(context->device.logical_device, shader->object_desdcriptor_pool, descriptor_set_count, object_state->descriptor_sets);
    if(result != VK_SUCCESS)
    {
