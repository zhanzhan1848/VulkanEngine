...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name, struct platform_state* plat_state)
{
    ...
    // and make sure they exists. Validation layers should only be enabled on non-release builds
#if defined(_DEBUG)
    ...
    // Create sync objects
    context.image_available_semaphores = darray_reserve(VkSemaphore, context.swapchain.max_frame_in_flight);
    context.queue_complete_semaphores = darray_reserve(VkSemaphore, context.swapchain.max_frame_in_flight);
    context.in_flight_fences = darray_reserve(vulkan_fence, context.swapchain.max_frame_in_flight);
    ...
    // In flight fences should not yet exist at this point, so clear the list. These are stroed in poiinters
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend)
{
    ...
    // Destroy in the apposite order of creation
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time)
{
    ...
    // Make sure the previous frams is not using this image(i.e. its fence is being waited on)
    ...
    // Command buffer(s) to the executed
    submit_info.commandBufferCount = 1;
    submit_info.pCommandBuffers = &command_buffer->handle;

    // The semaphore(s) to the signaled when the queue is complete.
...
b8 recreate_swapchain(renderer_backend* backend)
{
    ...
    // Makr as recreating if the dimensions are valid
    ...
    // Syne the framebuffer size with the cached sizes
