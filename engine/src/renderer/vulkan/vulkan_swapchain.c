...
)
{
    ...
    present_info.pWaitSemaphores = &render_compete_semaphore;
    ...
    context->current_frame = (context->current_frame + 1) % swapchain->max_frame_in_flight;
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    // Clamp to the value allowed bu the GPU
    ...
    // Setup the queue familt indices
